// ========================================
// ADMIN OBJECT
// ========================================

const ADMIN_CREDENTIAL = { username: "admin", password: "qlcl@123" };

const admin = {
  auth: false,
  selected: [],
  editId: null,
  filteredData: [],
  tempSessionData: null,
  currentSessionTab: "form1",

  async toggleView() {
    if (this.auth) {
      app.showAdmin();
      await this.refresh();
    } else {
      document.getElementById("loginModal").classList.remove("hidden");
    }
  },

  handleLogin(event) {
    event.preventDefault();
    const username = document.getElementById("username").value.trim();
    const password = document.getElementById("password").value.trim();
    if (
      username === ADMIN_CREDENTIAL.username &&
      password === ADMIN_CREDENTIAL.password
    ) {
      this.auth = true;
      document.getElementById("rememberLogin").checked
        ? localStorage.setItem("BYT_ADMIN_AUTH", "true")
        : localStorage.removeItem("BYT_ADMIN_AUTH");
      this.closeLogin();
      this.toggleView();
    } else {
      document.getElementById("loginError").classList.remove("hidden");
    }
  },

  closeLogin() {
    document.getElementById("loginModal").classList.add("hidden");
    document.getElementById("loginError").classList.add("hidden");
  },
  logout() {
    this.auth = false;
    localStorage.removeItem("BYT_ADMIN_AUTH");
    app.showPublic();
  },
  openConfig() {
    document.getElementById("scriptUrlInput").value = APPS_SCRIPT_URL;
    document.getElementById("configModal").classList.remove("hidden");
  },
  saveConfig() {
    APPS_SCRIPT_URL = document.getElementById("scriptUrlInput").value.trim();
    localStorage.setItem("BYT_SCRIPT_URL", APPS_SCRIPT_URL);
    document.getElementById("configModal").classList.add("hidden");
    app.showToast("Thành công", "Đã lưu cấu hình Google Sheets");
  },

  openSessionConfig() {
    if (!SESSION_CONFIG)
      SESSION_CONFIG = {
        form1: { kieu_khao_sat: "", nguoipv: "", nguoi_tra_loi: "" },
        form2: { kieu_khao_sat: "", nguoipv: "", nguoi_tra_loi: "" },
        form3: { kieu_khao_sat: "" },
      };
    this.tempSessionData = JSON.parse(JSON.stringify(SESSION_CONFIG));
    document.getElementById("sessionModal").classList.remove("hidden");
    this.switchSessionTab("form1");
  },

  switchSessionTab(formId) {
    this.saveCurrentTabToTemp();
    ["form1", "form2", "form3"].forEach((id) => {
      const btn = document.getElementById(`tab-${id}`);
      if (id === formId) {
        btn.classList.add("bg-white", "shadow", "text-teal-700");
        btn.classList.remove("text-gray-500", "hover:text-gray-700");
      } else {
        btn.classList.remove("bg-white", "shadow", "text-teal-700");
        btn.classList.add("text-gray-500", "hover:text-gray-700");
      }
    });
    this.currentSessionTab = formId;
    const container = document.getElementById("session-fields-container");
    container.innerHTML = "";
    const struct =
      formId === "form1"
        ? form1Structure
        : formId === "form2"
          ? form2Structure
          : form3Structure;
    const data = this.tempSessionData[formId];
    container.appendChild(
      this.createSessionSelect(
        struct.demographics.find((f) => f.id === "kieu_khao_sat"),
        data.kieu_khao_sat,
      ),
    );
    if (formId !== "form3") {
      container.appendChild(
        this.createSessionSelect(
          struct.demographics.find((f) => f.id === "nguoipv"),
          data.nguoipv,
        ),
      );
      container.appendChild(
        this.createSessionSelect(
          struct.demographics.find((f) => f.id === "nguoi_tra_loi"),
          data.nguoi_tra_loi,
        ),
      );
    }
  },

  createSessionSelect(fieldObj, currentValue) {
    const div = document.createElement("div");
    let optionsHtml = `<option value="">-- Bỏ trống (Người dùng tự điền) --</option>`;
    fieldObj.options.forEach((opt) => {
      optionsHtml += `<option value="${opt}" ${opt === currentValue ? "selected" : ""}>${opt}</option>`;
    });
    div.innerHTML = `<label class="block text-sm font-medium mb-1">${fieldObj.label}</label><select id="ses_${fieldObj.id}" class="w-full border border-gray-300 p-2.5 rounded-lg focus:ring-2 focus:ring-teal-500 outline-none bg-white text-sm">${optionsHtml}</select>`;
    return div;
  },

  saveCurrentTabToTemp() {
    if (!this.tempSessionData) return;
    const tab = this.currentSessionTab;
    const kieuEl = document.getElementById("ses_kieu_khao_sat");
    if (kieuEl) this.tempSessionData[tab].kieu_khao_sat = kieuEl.value;
    if (tab !== "form3") {
      const pvEl = document.getElementById("ses_nguoipv");
      const tlEl = document.getElementById("ses_nguoi_tra_loi");
      if (pvEl) this.tempSessionData[tab].nguoipv = pvEl.value;
      if (tlEl) this.tempSessionData[tab].nguoi_tra_loi = tlEl.value;
    }
  },

  async saveSessionConfig() {
    this.saveCurrentTabToTemp();
    try {
      app.showToast("Đang xử lý", "Đang lưu cấu hình lên máy chủ...");
      await db.call("saveConfig", { data: this.tempSessionData });
      SESSION_CONFIG = JSON.parse(JSON.stringify(this.tempSessionData));
      localStorage.setItem(
        "BYT_SESSION_CONFIG",
        JSON.stringify(SESSION_CONFIG),
      );
      document.getElementById("sessionModal").classList.add("hidden");
      app.updateSessionUI();
      app.showToast("Thành công", "Đã lưu cấu hình phiên cho các mẫu!");
    } catch (error) {
      alert("Lỗi khi lưu lên Google Sheet: " + error.message);
    }
  },

  async refresh() {
    this.selected = [];
    const tbody = document.getElementById("adminTableBody");
    tbody.innerHTML =
      '<tr><td colspan="7" class="text-center p-4 text-gray-500">Đang tải dữ liệu từ Sheet...</td></tr>';
    try {
      const data = await db.getAll(true);
      this.renderStats(data);
      this.filterTable();
      this.updateBulkUI();
    } catch (error) {
      tbody.innerHTML =
        '<tr><td colspan="7" class="text-center p-4 text-red-500">Lỗi: ' +
        error.message +
        "</td></tr>";
    }
  },

  renderStats(data) {
    document.getElementById("stat-total").textContent = data.length;
    document.getElementById("stat-form1").textContent = data.filter(
      (d) => d.type === "form1",
    ).length;
    document.getElementById("stat-form2").textContent = data.filter(
      (d) => d.type === "form2",
    ).length;
    let sum = 0,
      count = 0;
    data.forEach((record) => {
      if (record.python_data) {
        try {
          const g1 = parseInt(JSON.parse(record.python_data).G1);
          if (!isNaN(g1) && g1 > 0) {
            sum += g1;
            count++;
          }
        } catch (e) {}
      }
    });
    document.getElementById("stat-avg").textContent =
      (count > 0 ? (sum / count).toFixed(1) : "0.0") + "%";
  },

  filterTable() {
    const searchText = document
      .getElementById("searchInput")
      .value.toLowerCase();
    const formType = document.getElementById("filterFormType").value;
    const dateFrom = document.getElementById("filterDateFrom").value;
    const dateTo = document.getElementById("filterDateTo").value;
    const scoreFilter = document.getElementById("filterScore").value;

    this.filteredData = db.cache.filter((record) => {
      const matchesType = formType === "all" || record.type === formType;
      const matchesSearch =
        !searchText ||
        JSON.stringify(record).toLowerCase().includes(searchText);
      let matchesDate = true;
      if (dateFrom || dateTo) {
        const recordDate = new Date(record.timestamp);
        recordDate.setHours(0, 0, 0, 0);
        if (dateFrom) {
          const fromDate = new Date(dateFrom);
          fromDate.setHours(0, 0, 0, 0);
          if (recordDate < fromDate) matchesDate = false;
        }
        if (dateTo && matchesDate) {
          const toDate = new Date(dateTo);
          toDate.setHours(23, 59, 59, 999);
          if (recordDate > toDate) matchesDate = false;
        }
      }
      let matchesScore = true;
      if (scoreFilter !== "all") {
        matchesScore = false;
        if (record.python_data) {
          try {
            const pyData = JSON.parse(record.python_data);
            if (scoreFilter === "low_g1") {
              const g1 = parseInt(pyData.G1);
              if (!isNaN(g1) && g1 < 50) matchesScore = true;
            } else if (scoreFilter === "has_low") {
              const struct =
                record.type === "form1"
                  ? form1Structure
                  : record.type === "form2"
                    ? form2Structure
                    : form3Structure;
              let foundLow = false;
              for (const section of struct.sections) {
                for (const q of section.questions) {
                  if (!q.isCost) {
                    const val = parseInt(pyData[q.id]);
                    if (val >= 1 && val <= 3) {
                      foundLow = true;
                      break;
                    }
                  }
                }
                if (foundLow) break;
              }
              if (foundLow) matchesScore = true;
            }
          } catch (e) {}
        }
      }
      return matchesType && matchesSearch && matchesDate && matchesScore;
    });
    this.renderTable(this.filteredData);
  },

  renderTable(data) {
    const tbody = document.getElementById("adminTableBody");
    const emptyState = document.getElementById("empty-state");
    tbody.innerHTML = "";
    if (!data || data.length === 0) {
      emptyState.classList.remove("hidden");
      return;
    }
    emptyState.classList.add("hidden");

    data.forEach((record) => {
      const tr = document.createElement("tr");
      tr.className = "hover:bg-gray-50 border-b";
      const status = record.selenium_status || "READY";
      const statusColor =
        status === "DONE"
          ? "bg-green-100 text-green-700"
          : "bg-yellow-100 text-yellow-700";
      let score = "N/A";
      let hasLowScoreWarning = false;
      if (record.python_data) {
        try {
          const pyData = JSON.parse(record.python_data);
          score = pyData.G1 ? pyData.G1 + "%" : "N/A";
          const struct =
            record.type === "form1"
              ? form1Structure
              : record.type === "form2"
                ? form2Structure
                : form3Structure;
          for (const section of struct.sections) {
            for (const q of section.questions) {
              if (!q.isCost) {
                const val = parseInt(pyData[q.id]);
                if (val >= 1 && val <= 3) {
                  hasLowScoreWarning = true;
                  break;
                }
              }
            }
            if (hasLowScoreWarning) break;
          }
        } catch (e) {}
      }
      const warningIcon = hasLowScoreWarning
        ? `<i class="fas fa-exclamation-triangle text-red-500 ml-1 text-xs animate-pulse" title="Có câu hỏi ≤ 3 điểm"></i>`
        : "";
      let dept =
        record["5. Khoa nằm điều trị trước ra viện"] ||
        record["Khoa điều trị ngoại trú"] ||
        record["3. Khoa phòng của nhân viên"] ||
        "";
      const checked = this.selected.includes(record.id) ? "checked" : "";

      tr.innerHTML = `
        <td class="admin-table-cell p-4 text-center"><input type="checkbox" ${checked} onchange="admin.toggle('${record.id}')" class="w-3.5 h-3.5 md:w-4 md:h-4 rounded text-teal-600"></td>
        <td class="admin-table-cell p-4"><div class="font-bold text-gray-900 flex items-center">${record.id} ${warningIcon}</div><div class="text-[10px] md:text-xs text-gray-400">${new Date(record.timestamp).toLocaleString("vi-VN", { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" })}</div></td>
        <td class="admin-table-cell p-4"><div class="text-[11px] md:text-sm truncate max-w-[120px] md:max-w-[200px] text-gray-600" title="${dept}">${dept || "---"}</div></td>
        <td class="admin-table-cell p-4"><span class="bg-gray-100 text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 md:py-1 rounded font-bold text-gray-600 whitespace-nowrap">${record.type === "form1" ? "Nội trú" : record.type === "form2" ? "Ngoại trú" : "Nhân viên"}</span></td>
        <td class="admin-table-cell p-4 text-center"><span class="${statusColor} text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 md:py-1 rounded font-bold whitespace-nowrap">${status}</span></td>
        <td class="admin-table-cell p-4 text-center font-bold text-teal-600 text-xs md:text-sm"><span class="${score !== "N/A" && parseInt(score) < 50 ? "text-red-600" : ""}">${score}</span></td>
        <td class="admin-table-cell p-4 text-right whitespace-nowrap"><button onclick="admin.openEditModal('${record.id}')" class="admin-action-btn text-blue-500 hover:bg-blue-50 p-1.5 md:p-2 rounded transition" title="Sửa chi tiết"><i class="fas fa-edit"></i></button><button onclick="admin.del('${record.id}')" class="admin-action-btn text-red-500 hover:bg-red-50 p-1.5 md:p-2 rounded transition ml-1" title="Xóa"><i class="fas fa-trash"></i></button></td>`;
      tbody.appendChild(tr);
    });
  },

  toggle(id) {
    const index = this.selected.indexOf(id);
    index > -1 ? this.selected.splice(index, 1) : this.selected.push(id);
    this.updateBulkUI();
  },
  toggleSelectAll() {
    this.selected = document.getElementById("selectAll").checked
      ? this.filteredData.map((record) => record.id)
      : [];
    this.filterTable();
    this.updateBulkUI();
  },
  updateBulkUI() {
    const count = this.selected.length;
    document.getElementById("selected-count").textContent = count;
    document.getElementById("bulk-actions").style.display =
      count > 0 ? "flex" : "none";
  },
  async del(id) {
    if (!confirm("Xóa phiếu này?")) return;
    try {
      await db.delete([id]);
      app.showToast("Thành công", "Đã xóa phiếu");
      await this.refresh();
    } catch (error) {
      app.showToast("Lỗi", "Không thể xóa: " + error.message, "error");
    }
  },
  async deleteBulk() {
    if (!confirm(`Xóa ${this.selected.length} phiếu đã chọn?`)) return;
    try {
      await db.delete(this.selected);
      app.showToast("Thành công", `Đã xóa ${this.selected.length} phiếu`);
      await this.refresh();
    } catch (error) {
      app.showToast("Lỗi", "Không thể xóa: " + error.message, "error");
    }
  },
  openBulkEdit() {
    document.getElementById("bulk-edit-count").textContent =
      this.selected.length;
    document.getElementById("bulkEditModal").classList.remove("hidden");
  },
  setBulkScore(score) {
    document.getElementById("bulk-score-val").value = score;
    document
      .querySelectorAll(".bulk-btn")
      .forEach((btn) => btn.classList.remove("active"));
    event.target.classList.add("active");
  },

  async applyBulkEdit() {
    const section = document.getElementById("bulk-section").value;
    const score = document.getElementById("bulk-score-val").value;
    if (
      !confirm(
        `Sửa điểm thành ${score} cho ${section === "all" ? "toàn bộ câu hỏi" : "nhóm " + section} của ${this.selected.length} phiếu?`,
      )
    )
      return;
    try {
      const recordsToUpdate = db.cache.filter((r) =>
        this.selected.includes(r.id),
      );
      const updates = {};
      recordsToUpdate.forEach((record) => {
        if (!record.python_data) return;
        try {
          const pythonData = JSON.parse(record.python_data);
          const formStruct =
            record.type === "form1"
              ? form1Structure
              : record.type === "form2"
                ? form2Structure
                : form3Structure;
          formStruct.sections.forEach((sect) => {
            const sectionLetter = sect.title.split(".")[0].trim();
            if (section === "all" || section === sectionLetter) {
              sect.questions.forEach((q) => {
                if (!q.isCost) pythonData[q.id] = score;
              });
            }
          });
          if (!updates[record.id]) updates[record.id] = {};
          updates[record.id]["python_data"] = JSON.stringify(pythonData);
        } catch (e) {}
      });
      await Promise.all(
        Object.entries(updates).map(([id, upd]) => db.update([id], upd)),
      );
      document.getElementById("bulkEditModal").classList.add("hidden");
      app.showToast("Thành công", `Đã cập nhật ${this.selected.length} phiếu`);
      await this.refresh();
    } catch (error) {
      app.showToast("Lỗi", "Không thể cập nhật: " + error.message, "error");
    }
  },

  exportExcel() {
    document.getElementById("exportChoiceModal").classList.remove("hidden");
    document.getElementById("exportSheetSelect").value = "";
  },

  handleExportChoice() {
    const selectedSheet = document.getElementById("exportSheetSelect").value;
    if (!selectedSheet) {
      alert("Vui lòng chọn một mẫu để xuất.");
      return;
    }
    document.getElementById("exportChoiceModal").classList.add("hidden");

    const dataToExport = db.cache.filter(
      (record) => record.type === selectedSheet,
    );
    if (dataToExport.length === 0) {
      alert(`Mẫu chưa có dữ liệu.`);
      return;
    }

    // XÓA TẬN GỐC CÁC BIẾN UI SINH RA ĐỂ EXCEL Y HỆT GOOGLE SHEET
    const cleanData = dataToExport.map((item) => {
      const copy = { ...item };
      delete copy.id;
      delete copy.timestamp;
      delete copy.type;
      delete copy.selenium_status;
      delete copy._ui_type;
      return copy;
    });

    try {
      const worksheet = XLSX.utils.json_to_sheet(cleanData);
      const workbook = XLSX.utils.book_new();
      const sheetNameMap = {
        form1: "Mau 1 - Noi Tru",
        form2: "Mau 2 - Ngoai Tru",
        form3: "Mau 3 - Nhan Vien",
      };
      const sheetName = sheetNameMap[selectedSheet] || "Data";
      XLSX.utils.book_append_sheet(workbook, worksheet, sheetName);
      XLSX.writeFile(
        workbook,
        `BaoCao_${sheetName.replace(/ /g, "_")}_${new Date().toISOString().slice(0, 10)}.xlsx`,
      );
      app.showToast(
        "Thành công",
        `Đã xuất ${cleanData.length} phiếu của ${sheetName}.`,
      );
    } catch (e) {
      app.showToast("Lỗi", "Không thể tạo file Excel: " + e.message, "error");
    }
  },

  openEditModal(id) {
    this.editId = String(id);
    const record = db.cache.find((r) => String(r.id) === String(id));
    if (!record) {
      alert("Không tìm thấy dữ liệu phiếu");
      return;
    }
    document.getElementById("edit-id-display").textContent = record.id;
    this.renderEditForm(record);
    document.getElementById("editRecordModal").classList.remove("hidden");
  },

  renderEditForm(record) {
    const container = document.getElementById("edit-form-container");
    container.innerHTML = "";
    const recordData = { ...record, ...JSON.parse(record.python_data || "{}") };
    const formStruct =
      record.type === "form1"
        ? form1Structure
        : record.type === "form2"
          ? form2Structure
          : form3Structure;

    const demoDiv = document.createElement("div");
    demoDiv.className =
      "mb-6 bg-white p-4 rounded-lg shadow-sm border border-gray-200";
    demoDiv.innerHTML = `<h3 class="font-bold text-teal-700 mb-4 border-b pb-2">I. THÔNG TIN HÀNH CHÍNH</h3>`;
    const demoGrid = document.createElement("div");
    demoGrid.className = "grid grid-cols-1 md:grid-cols-2 gap-4";
    formStruct.demographics.forEach((field) => {
      const val = recordData[field.id] || "";
      const wrapper = document.createElement("div");
      wrapper.className =
        field.width === "full" ? "col-span-1 md:col-span-2" : "col-span-1";
      wrapper.innerHTML = this.renderInputControl(field, val);
      demoGrid.appendChild(wrapper);
    });
    demoDiv.appendChild(demoGrid);
    container.appendChild(demoDiv);

    formStruct.sections.forEach((section) => {
      const sectionDiv = document.createElement("div");
      sectionDiv.className =
        "mb-6 bg-white p-4 rounded-lg shadow-sm border border-gray-200";
      sectionDiv.innerHTML = `<h3 class="font-bold text-gray-800 mb-3 bg-gray-50 p-2 rounded">${section.title}</h3>`;
      section.questions.forEach((summaryQuestion) => {
        summaryQuestion.mapToHeaders.forEach((detailedHeader) => {
          const questionDiv = document.createElement("div");
          questionDiv.className =
            "mb-4 pb-4 border-b last:border-b-0 last:pb-0 last:mb-0";
          const value = recordData[detailedHeader] || "";
          const isLow =
            !summaryQuestion.isCost &&
            parseInt(value) <= 3 &&
            parseInt(value) > 0;
          const bgClass = isLow ? "bg-red-50 border border-red-200" : "";
          let inputHTML = "";
          if (summaryQuestion.isCost) {
            inputHTML = this.renderInputControl(
              {
                id: `detail_${detailedHeader}`,
                label: detailedHeader,
                type: "radio",
                options: [
                  "1. Rất đắt so với chất lượng",
                  "2. Đắt hơn so với chất lượng",
                  "3. Tương xứng so với chất lượng",
                  "4. Rẻ hơn so với chất lượng",
                  "5. Không tự chi trả / Không biết",
                  "6. Ý kiến khác",
                ],
              },
              value,
              true,
            );
          } else {
            const radioName = `edit_detail_${detailedHeader.replace(/[^a-zA-Z0-9]/g, "_")}`;
            inputHTML = `<div class="flex flex-wrap gap-2 mt-2">${[
              1, 2, 3, 4, 5,
            ]
              .map((v) => {
                const isChecked = String(value) === String(v) ? "checked" : "";
                const activeColor =
                  v <= 3 && v > 0
                    ? "peer-checked:bg-red-500 peer-checked:text-white peer-checked:border-red-500"
                    : "peer-checked:bg-teal-600 peer-checked:text-white peer-checked:border-teal-600";
                return `<div class="relative"><input type="radio" name="${radioName}" id="${radioName}_${v}" value="${v}" ${isChecked} class="peer hidden"><label for="${radioName}_${v}" class="w-10 h-10 rounded-full flex items-center justify-center font-bold cursor-pointer transition-all border border-transparent select-none bg-gray-100 text-gray-600 hover:bg-gray-200 ${activeColor} peer-checked:ring-2">${v}</label></div>`;
              })
              .join("")}</div>`;
          }
          questionDiv.innerHTML = `<div class="${bgClass} p-2 rounded transition-colors duration-300"><div class="font-medium text-sm text-gray-700 mb-1">${detailedHeader}</div>${inputHTML}</div>`;
          sectionDiv.appendChild(questionDiv);
        });
      });
      container.appendChild(sectionDiv);
    });

    const footerDiv = document.createElement("div");
    footerDiv.className =
      "mb-6 bg-white p-4 rounded-lg shadow-sm border border-gray-200";
    footerDiv.innerHTML = `<h3 class="font-bold text-teal-700 mb-4 border-b pb-2">THÔNG TIN KHÁC</h3>`;
    const footerStack = document.createElement("div");
    footerStack.className = "space-y-4";
    formStruct.footer.forEach((field) => {
      const wrapper = document.createElement("div");
      wrapper.innerHTML = this.renderInputControl(
        field,
        recordData[field.id] || "",
      );
      footerStack.appendChild(wrapper);
    });
    footerDiv.appendChild(footerStack);
    container.appendChild(footerDiv);
  },

  renderInputControl(field, currentValue, hideLabel = false) {
    const name = `edit_${field.id}`;
    let html = hideLabel
      ? ""
      : `<label class="block text-xs font-bold text-gray-500 mb-1 uppercase">${field.label || field.text}</label>`;
    if (field.type === "select") {
      html += `<select name="${name}" class="w-full border p-2 rounded text-sm bg-gray-50 focus:bg-white outline-none"><option value="">-- Chọn --</option>${field.options.map((opt) => `<option value="${opt}" ${opt == currentValue ? "selected" : ""}>${opt}</option>`).join("")}</select>`;
    } else if (field.type === "radio") {
      html += `<div class="space-y-2 mt-1">${field.options.map((opt, i) => `<div class="flex items-center"><input type="radio" id="${name}_opt_${i}" name="${name}" value="${opt}" ${opt == currentValue ? "checked" : ""} class="w-4 h-4 text-teal-600 cursor-pointer"><label for="${name}_opt_${i}" class="ml-2 block text-sm text-gray-700 cursor-pointer">${opt}</label></div>`).join("")}</div>`;
    } else if (field.type === "textarea") {
      html += `<textarea name="${name}" rows="3" class="w-full border p-2 rounded text-sm bg-gray-50 focus:bg-white outline-none">${currentValue}</textarea>`;
    } else {
      html += `<input type="${field.type || "text"}" name="${name}" value="${currentValue}" class="w-full border p-2 rounded text-sm bg-gray-50 focus:bg-white outline-none">`;
    }
    return html;
  },

  async saveSingleEdit() {
    if (!this.editId) return;
    const record = db.cache.find((r) => String(r.id) === String(this.editId));
    if (!record) return;
    try {
      let pythonData = JSON.parse(record.python_data || "{}");
      const formStruct =
        record.type === "form1"
          ? form1Structure
          : record.type === "form2"
            ? form2Structure
            : form3Structure;
      const modalContainer = document.getElementById("edit-form-container");
      const updates = {};
      const getVal = (name) => {
        const r = modalContainer.querySelector(`input[name="${name}"]:checked`);
        return r
          ? r.value
          : modalContainer.querySelector(`[name="${name}"]`)?.value || "";
      };

      formStruct.demographics.forEach((field) => {
        const val = getVal(`edit_${field.id}`);
        pythonData[field.id] = val;
        if (field.mapToHeader) updates[field.mapToHeader] = val;
      });
      formStruct.sections.forEach((s) =>
        s.questions.forEach((q) => {
          let firstValue = null;
          q.mapToHeaders.forEach((header, i) => {
            const val = q.isCost
              ? getVal(`edit_detail_${header}`)
              : getVal(`edit_detail_${header.replace(/[^a-zA-Z0-9]/g, "_")}`);
            updates[header] = val;
            if (i === 0) firstValue = val;
          });
          pythonData[q.id] = firstValue;
        }),
      );
      formStruct.footer.forEach((field) => {
        const val = getVal(`edit_${field.id}`);
        pythonData[field.id] = val;
        if (field.mapToHeader) updates[field.mapToHeader] = val;
      });

      updates["python_data"] = JSON.stringify(pythonData);
      await db.update([this.editId], updates);
      document.getElementById("editRecordModal").classList.add("hidden");
      app.showToast("Thành công", "Đã cập nhật dữ liệu Google Sheet");
      await this.refresh();
    } catch (error) {
      app.showToast("Lỗi", "Không thể cập nhật: " + error.message, "error");
    }
  },
};

document.addEventListener("DOMContentLoaded", () => {
  if (localStorage.getItem("BYT_ADMIN_AUTH") === "true") admin.auth = true;
  app.init();
});
